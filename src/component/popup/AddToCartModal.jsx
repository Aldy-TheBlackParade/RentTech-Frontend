...
import { createCart } from "../../service/category";
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (response) {
        ...
        onClose(); // Tutup modal
        navigate("/cart"); // Pindah ke halaman cart
      }
    } catch (err) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm">
      <div className="bg-white rounded-2xl p-6 w-full max-w-md shadow-2xl">
        <h3 className="text-xl font-bold mb-4">Tambah ke Keranjang</h3>
        ...
        <div className="flex gap-3 mt-8">
          ...
          >
            {loading ? "Memproses..." : "Masuk Keranjang"}
          </button>
        </div>
      </div>
    </div>
